...
STATE *bfs(STATE *start,STATE *target) {
  ...
  parent_list[0] = start;
  parent_amount++;
  book = insert_to_book(book,arr_to_num(start->data));
  while(parent_amount) {
	...
	for(int i = 0;i < parent_amount;i++) {
	  ...
	  for(int j = 0;j < 4;j++) {
		...
		// 判断是否为目标状态节点
		//show(new_state);
		if(is_same_state(new_state,target)) {
		  return new_state;
		}
		// 若该状态在book表中则跳过
		if( is_exist(book,arr_to_num(new_state->data)) ) {
		  free(new_state);
		  continue;
		}
		// 将该状态节点加入book表中
		book = insert_to_book(book,arr_to_num(new_state->data));
		// 将该状态节点加入临时列表中
